...
(function() {
  ...
  const fallbackData = {
    ...
    searchResults: [
      { id: 1, carabiniereId: 1, nome: 'Mario', cognome: 'Rossi', matricola: 'CC123456', dataInizio: '2025-04-23', dataFine: '2025-05-06', giorni: 14, tipo: 'Malattia', note: 'Influenza', reparto: 'Reparto Operativo' },
      { id: 2, carabiniereId: 2, nome: 'Giuseppe', cognome: 'Verdi', matricola: 'CC234567', dataInizio: '2025-04-15', dataFine: '2025-04-22', giorni: 8, tipo: 'Infortunio', note: 'Trauma alla caviglia', reparto: 'Nucleo Radiomobile' }
    ],
    history: [
      { id: 1, carabiniereId: 1, nome: 'Mario', cognome: 'Rossi', matricola: 'CC123456', dataInizio: '2025-03-10', dataFine: '2025-03-20', giorni: 11, tipo: 'Malattia', note: 'Influenza stagionale', reparto: 'Reparto Operativo' },
      { id: 2, carabiniereId: 1, nome: 'Mario', cognome: 'Rossi', matricola: 'CC123456', dataInizio: '2025-01-15', dataFine: '2025-01-25', giorni: 11, tipo: 'Malattia', note: 'Influenza', reparto: 'Reparto Operativo' },
      { id: 3, carabiniereId: 2, nome: 'Giuseppe', cognome: 'Verdi', matricola: 'CC234567', dataInizio: '2024-12-20', dataFine: '2024-12-27', giorni: 8, tipo: 'Infortunio', note: 'Distorsione caviglia', reparto: 'Nucleo Radiomobile' }
    ],
    users: [
      { id: 1, username: 'admin', isAdmin: true, email: 'admin@esempio.it' },
      { id: 2, username: 'operatore', isAdmin: false, email: 'operatore@esempio.it' }
    ],
    statistics: {
      summary: {
        totalMalattie: 12,
        totalInfortuni: 5,
        totalGiorniAssenza: 120,
        mediaGiorniPerAssenza: 7.5
      },
      thresholds: {
        carabinieriSopra90Giorni: 2,
        carabinieriSopra180Giorni: 1,
        carabinieriSopra240Giorni: 0
      },
      monthly: [
        { mese: 'Gennaio', assenze: 15 },
        { mese: 'Febbraio', assenze: 12 },
        { mese: 'Marzo', assenze: 18 },
        { mese: 'Aprile', assenze: 20 },
        { mese: 'Maggio', assenze: 14 },
        { mese: 'Giugno', assenze: 10 }
      ]
    }
  };
    ...
    try {
      ...
      try {
        ...
        if (isApiInfoMessage(data)) {
          ...
          } else if (resource.toString().includes('/api/history')) {
            fixedData = fallbackData.history;
            console.log('[API DATA FIX] Utilizzando dati cronologia di fallback');
          } else if (resource.toString().includes('/api/users')) {
            fixedData = fallbackData.users;
            console.log('[API DATA FIX] Utilizzando dati utenti di fallback');
          } else if (resource.toString().includes('/api/statistics/summary')) {
            fixedData = fallbackData.statistics.summary;
            console.log('[API DATA FIX] Utilizzando dati statistiche di fallback');
          } else if (resource.toString().includes('/api/statistics/thresholds')) {
            fixedData = fallbackData.statistics.thresholds;
            console.log('[API DATA FIX] Utilizzando dati soglie di fallback');
          } else if (resource.toString().includes('/api/statistics/monthly')) {
            fixedData = fallbackData.statistics.monthly;
            console.log('[API DATA FIX] Utilizzando dati mensili di fallback');
          }
